...
import { CORS, Env, HttpExceptionFilter, TransformInterceptor, setupSwagger } from '@jitera/common'
...
async function bootstrap() {
  ...
  fastify.register(multipart, { // Read File
  ...
  fastify.register(helmet, { // Header security
  ...
  const app = await NestFactory.create<NestFastifyApplication>(AppModule, fastify, { bufferLogs: true, rawBody: true }) // How app run
  ...
  app.enableCors({
    origin: config.get<Env>('env') === Env.PRODUCTION ? config.get<string | string[]>('server.whitelist') : '*', // Enable CORS
    ...CORS,
  })
