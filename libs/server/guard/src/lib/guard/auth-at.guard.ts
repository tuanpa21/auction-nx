...
export class JwtAtGuard extends AuthGuard('jwt') {
  ...
  handleRequest<TUser = any>(err: Error, user: any, info: any): TUser {
    if (info instanceof TokenExpiredError) {
      throw new GuardException('TOKEN_EXPIRED')
    }
    if (info instanceof NotBeforeError) {
      throw new GuardException('TOKEN_CLAIMS_BEFORE')
    }
    if (info instanceof JsonWebTokenError) {
      throw new GuardException('TOKEN_INVALID')
    }
    if (err || !user) throw new GuardException('UNAUTHORIZED_ACCESS_TOKEN')
