...
export class TransformInterceptor<T> implements NestInterceptor<T, IResponse<T>> {
  intercept(context: ExecutionContext, next: CallHandler): Observable<IResponse<T>> {
    return next.handle().pipe(map((res) => (isObject(res) ? this.transformResponse(res) : { data: res })))
  }
  ...
  transformResponse(response: Record<string, any>): IResponse<T> {
    return response.meta && response.data
      ...
      : { data: isArray(response) ? response.map(this.transformToPlain) : this.transformToPlain(response) }
  }
